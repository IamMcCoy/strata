...
test:  ## 단위 테스트 — 외부 의존 없음. Redis 없어도, API 키가 있어도 밖으로 나가지 않는다
	uv run pytest -q
...
test-integration: redis-up  ## 실제 Redis + 실제 SQLite 파일 + 실제 멀티프로세스로 검증
	uv run pytest -m integration tests/test_memory_integration.py tests/test_pipeline_integration.py -v

test-providers:  ## 실제 엔드포인트 검증 — base_url이 없으면 **유료 API로 나간다**
	uv run pytest -m integration tests/test_openai_integration.py -v
