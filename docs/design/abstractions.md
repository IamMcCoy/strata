...
"무엇이 관련 있는가"는 하나여야 하기 때문이다. **BM25**로 매기되 토큰이 아니라 **부분 문자열**로
센다: 한국어가 교착어라 단어 단위 비교('uv를' != 'uv')가 거의 다 빗나가서다. 그래서 문서 길이의
단위도 토큰 수가 아니라 문자 수다.

처음엔 단순 겹침(질의 단어가 몇 개 들어 있나)이었는데 오동작했다 — 점수의 최댓값이 질의 단어 수라
3단어 질의면 값이 네 가지뿐이고, 동점이 대량 발생해 안정 정렬이 **삽입 순서**로 잘랐다.
"가장 관련 있는 것"이 아니라 "가장 먼저 저장된 것"이 나왔고, 항목 10건에서도 그랬다.
BM25는 빈도·문서 길이·단어의 희소성을 함께 보므로 점수가 연속값이 되어 그 붕괴가 사라진다
(`tests/test_memory.py`의 네 회귀 테스트가 옛 알고리즘에서 전부 실패한다).

**남은 한계 — 알고 받아들인 것.**

- **여전히 전체 스캔이다.** SQLite는 `WHERE ns = ?`로 전부 꺼내고 Redis는 `hgetall`이다.
  실측(20,000건): InMemory 25ms, SQLite 64ms per `Agent.run()`. BM25가 단순 겹침보다 ~1.5배
  느리다(`.count()`는 조기 종료가 없다) — 정확성과 바꾼 값이고, 이 규모에서 아프면 그때 고친다.
- **의미 검색이 아니다.** '결제 실패'로 '구매 오류'를 찾지 못한다. 동의어·의역이 필요해지면
  같은 `Memory` 인터페이스로 `VectorMemory`를 붙인다 — 저장소별 전문검색(FTS5/tsvector)으로
  흩지 말 것. 구현마다 결과가 갈린다.
- **내용이 진짜로 같은 항목들 사이에는 순서가 없다.** 어떤 점수 함수로도 가를 수 없다.
...
**대화를 Memory에 쌓지 말 것.** `retrieve`는 BM25 점수이고 **순서 개념이 없다** —
"3번째 턴에서 뭐라고 했는지"를 복원할 수 없다. 점수 함수를 아무리 잘 만들어도 마찬가지다:
순서는 다른 자료구조가 필요한 문제이지 더 좋은 검색이 필요한 문제가 아니다.
게다가 매 턴이 쌓이면 "네 알겠습니다" 같은 상투구가 모든 항목에 퍼져 IDF를 0으로 만들고,
진짜 기억("사용자는 uv를 쓴다")이 그 사이에 묻힌다. 둘은 층을 이룬다:
...
#### 대화가 길어지면 — 자르기는 앱의 몫이지만 아무 데서나 자를 수는 없다

**코어는 자르지 않는다.** `Agent.run`은 받은 것을 그대로 이어붙인다:

```python
messages=[*(history or []), {'role': 'user', 'content': task}]
```

의도적이다. 자르는 정책이 앱마다 다르고(최근 N턴 / 토큰 예산 / 요약 / 중요 턴 고정),
토큰을 세려면 모델별 토크나이저가 필요해 런타임 의존성 0이 깨지며, 어느 턴이 중요한지는
대화를 소유한 앱이 더 잘 안다.

**넘치면 무슨 일이 나나.** 프로바이더가 `400 context_length_exceeded`를 내고,
`ProviderError` → `status='failed'`로 변환되어 **지금까지의 답과 함께** 돌아온다(ADR-0013).
크래시는 나지 않는다. 다만 **오분류된다**: 이건 재시도해도 영원히 안 되는 400인데
일시적 인프라 오류와 같은 칸에 들어간다. `FallbackProvider`를 붙여뒀다면 다음 프로바이더에서도
똑같이 실패하며 비용만 쓴다. 자르기를 앱이 하지 않으면 이 비용을 치른다.

**함정 — tool 왕복은 쌍이다.** `history[-20:]`처럼 순진하게 자르면 조용히 깨진다:

```python
{'role': 'assistant', 'content': ..., 'tool_calls': [{'id': 'call_1', ...}]}   # ← 이걸 자르고
{'role': 'tool', 'tool_call_id': 'call_1', 'content': '...'}                   # ← 이것만 남기면 400
```
...
**D를 권한다.** history(순서 있는 원문)를 짧게 유지하고 영속시킬 것은 Memory(순서 없는 사실)로
옮기는 것 — 두 저장소를 나눈 이유가 정확히 이것이다. 위의 층 다이어그램이 그 흐름이다.
